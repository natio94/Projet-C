/**
 * Projet C Dataframe
 * Ce fichier contient les fonctions pour manipuler les dataframes.
...
void add_row_dataframe(CDataframe *df) {
    ...
    for (int i = 0; i < df->numb_columns; i++) {
        switch (df->columns[i]->column_type) {
            case UINT: {
                unsigned int value;
                printf("Entrez la valeur de la ligne %d pour la colonne %s (%d): ", df->columns[i]->TL,
                       df->columns[i]->title, i);
                scanf("%u", &value);
                insert_values(df->columns[i], &value);
                break;
            }
